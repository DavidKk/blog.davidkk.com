...
HTML5 没有使用 SGML 或者 XHTML，他是一个全新的东西，因此你不需要参考 DTD，对于 HTML5，你仅需放置下面的文档类型代码告诉浏览器识别这是 HTML5 文档，若指明 `<!DOCTYPE html>` (不区分大小写)，则浏览器不会认为该文档为 HTML 文档，会导致 HTML5 的标签不能正常工作。 
...
其中页面可能被缓存了，检测的仍然可能是旧的 manifest 清单，若出现该情况则直接把 manifest 指向地址修改或加上版本号即可。
